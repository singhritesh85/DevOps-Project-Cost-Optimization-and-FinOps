...
if [ "$TIME" -ge 2000 ] || [ "$TIME" -lt 0730 ]; then
    ...
    if [ -z "$ACCOUNT_ID" ] || [ "$ACCOUNT_ID" = "None" ]; then
        echo "ERROR: Failed to fetch AWS Account ID. Check your credentials." >> "$LOG_FILE"
        ACCOUNT_ID="UNKNOWN"
    else
    ...
    # ========================================================
    # Get all AWS regions safely
    # ========================================================
    REGIONS=$(aws ec2 describe-regions --query 'Regions[].RegionName' --output text 2>/dev/null)

    if [ -z "$REGIONS" ]; then
        echo "ERROR: Failed to fetch AWS regions. Check permissions." >> "$LOG_FILE"
    else
        # ========================================================
        ...
        for REGION in $REGIONS; do
            ...
            if [ -n "$EC2_DATA" ] && [ "$EC2_DATA" != "None" ]; then
                ...
                echo "$EC2_DATA" | while read -r INSTANCE_ID INSTANCE_NAME; do
                    ...
                    if aws ec2 wait image-available --region "$REGION" --image-ids "$AMI_ID" 2>> "$LOG_FILE"; then
            ...
            # ====================================================
            RDS_DATA=$(aws rds describe-db-instances --region "$REGION" --query "DBInstances[?DBInstanceStatus=='available'].{ID:DBInstanceIdentifier,ARN:DBInstanceArn}" --output text 2>/dev/null)

            if [ -n "$RDS_DATA" ] && [ "$RDS_DATA" != "None" ]; then

                while read -r RDS_ARN RDS_ID; do
                    [ -z "$RDS_ID" ] && continue

                    # Check Environment tag (supports 'non-prod' or can be adjusted if needed)
                    ENVIRONMENT=$(aws rds list-tags-for-resource --region "$REGION" --resource-name "$RDS_ARN" --query "TagList[?Key=='Environment'].Value | [0]" --output text 2>/dev/null)
                    ...
                    if aws rds wait db-snapshot-available --region "$REGION" --db-snapshot-identifier "$SNAPSHOT_ID" 2>> "$LOG_FILE"; then
                    ...
                    # Delete RDS instance (skipping final internal snapshot since we made a custom pre-delete one)
