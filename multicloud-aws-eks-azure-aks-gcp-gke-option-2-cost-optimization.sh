                ...
                echo "$MATCHING_CLUSTERS" | jq -c '.[]?' | while read -r cluster_json; do
                    ...
                    echo "$NODE_POOLS" | jq -c '.[]?' | while read -r pool_json; do
                        ...
                        if [ "$INITIAL_COUNT" -gt 0 ]; then
                            echo "    -> Processing GKE Node Pool: $POOL_NAME (Initial Count: $INITIAL_COUNT)"

                            # Save state including configurations
                            ...
                            # Delete or resize based on user choice (using correct node-pools delete syntax)
                            if [ "$AUTOSCALING_ENABLED" = "true" ]; then
                                gcloud container node-pools delete "$POOL_NAME" --cluster="$CLUSTER_NAME" --project="$project" $LOCATION_FLAG --quiet > /dev/null 2>&1
                            else
